...
// 파일의 입출력 
package com.sist.io;
import java.io.*; // 제일 먼저 import
public class MainClass {
	public static void main(String[] args) {
		// FileInputStream fis=null; // 선언 (필요한 변수는 밖에 , 반드시 초기값 부여 => png,jpg,zip...) #
		// 모든 클래스의 초기값 ==> null
		FileReader fis=null;
		// 자바에서 저장 => txt는 Ansi로 저장된다 
		try {
			// 파일 읽기
			// fis=new FileInputStream("c:\\javaDev\\news1.txt"); // 일반 => FileInputStream #
			 fis=new FileReader("c:\\javaDev\\news1.txt"); // 한글이 포함된 파일 => FileReader
			 // 파일을 읽어서 도스창에 출력 
			 
			 int i=0; // 한글자 씩 읽는다 => 한글자씩 가지고 온다 , 문자번호 
			 // 캐릭터 말고 인트로 받아야한다 => 그리고 캐릭터로 형변환해서 출력 (한글이나 숫자는 1byte라 상관X)
			 while((i=fis.read())!=-1) { // -1 파일의 끝(EOF)
				 // System.out.println(i); // 한글을 문자번호로 가져옴
				 System.out.println((char)i); // ㄴ 이렇게 형변환했음 
			 }
